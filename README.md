...
⸻
# 📚 Course 2 – Cybersecurity Glossary
...
- **CIA Triad** (*Confidentiality, Integrity, Availability*): A model that helps inform how organizations consider risk when setting up systems and security policies.
...
<img width="921" height="518" alt="image" src="https://github.com/user-attachments/assets/685c8ab4-fd02-485f-8540-9cb3340e8497" />
...
<img width="610" height="742" alt="image" src="https://github.com/user-attachments/assets/16c88a09-07c2-498f-a18b-1c88ea3d0eb1" />



## 🧠 **Subnetting and CIDR – Easy Summary for Cybersecurity**
...
* Think of it as **"a network inside a network."**
* Helps isolate zones: 📦 internal, 🔒 restricted, 🌐 public (like DMZ).
* Increases **efficiency**, **speed**, and **security**.
* Used to create **security zones** like:
...
* Limits traffic within subnets (local traffic doesn’t go to the router).
* Segments **sensitive systems** from **public-facing** ones.
* Reduces **attack surface** – limits lateral movement in attacks.
* Allows **more control** over bandwidth and policies per subnet.

---

### 🔹 CIDR (Classless Inter-Domain Routing)

**CIDR Notation** = `IP address / Prefix`

Example: `192.168.1.0/24`

* `/24` = Network mask with **24 bits for network**, remaining **8 bits for hosts**.
* IP Range = `192.168.1.0 → 192.168.1.255` (total: **256 IPs**)

#### 🆚 CIDR vs Classful:

| Feature           | Classful             | CIDR                    |
| ----------------- | -------------------- | ----------------------- |
| Uses fixed ranges | Yes (Class A/B/C...) | No                      |
| Efficient?        | ❌ Wasteful           | ✅ More efficient        |
| Routing tables?   | Large                | Compact (fewer entries) |
| Flexibility       | Low                  | High                    |

---

### 📐 CIDR Benefits in Security

* Allows you to create **custom-sized networks** (no IP waste).
* Helps create **network segmentation** strategies (e.g., isolating IoT or HR devices).
* You don’t need to request new IPs from ISP — saves cost and setup time.
* Can work with:
...
* A company wants to separate **HR**, **IT**, and **Guest Wi-Fi**.
* Subnetting helps create:

  * `192.168.10.0/24` → HR
  * `192.168.20.0/24` → IT
  * `192.168.30.0/24` → Guests
    Each is isolated from the others = **better security + performance**.

---

### 🔧 Tools You Can Use

* **CIDR Calculator**: [https://ipaddressguide.com/cidr](https://ipaddressguide.com/cidr)
* Converts:

  * CIDR → IP range
  * IP range → CIDR
  * Helpful for planning firewall rules and routing

---

### 🔐 Summary: Why Subnetting Matters in Cybersecurity

| Benefit             | Explanation                                 |
| ------------------- | ------------------------------------------- |
| Network isolation   | Stops threats from spreading laterally      |
| Efficient IP usage  | Custom IP ranges → No waste                 |
| Better routing      | Smaller routing tables                      |
| Improved monitoring | Easier to detect unusual traffic per subnet |
| Access control      | Firewalls and ACLs can be subnet-specific   |
...
ack where data packets are manipulated in transit
...
guration: A documented set of speci

cations within a system that is
...
rewalls: So
ware
rewalls that are hosted by the cloud service
...
ack that targets a network or server and
oods
...
ack: A type of denial of service a

ack that

uses multiple devices or servers located in di

erent locations to

ood the target

network with unwanted tra
c
...
les from one device to another over a

network
Firewall: A network security device that monitors tra

c to or from your network
...
IEEE 802.11 (Wi-Fi): A set of standards that de

ne communication for wireless LANs
...
an a
acker repeatedly sending ICMP request packets to a network server
...
Internet Protocol (IP) address: A unique string of characters that identi
es the
...
ack performed when an a

acker changes the source IP of a
data packet to impersonate an authorized system and gain access to a network
L
Local area network (LAN): A network that spans small areas like an o

ce building, a

school, or a home
M
Media Access Control (MAC) address: A unique alphanumeric identi
...
ack where a malicious actor places themselves in the middle of

an authorized connection and intercepts or alters the data in transit
P
Packet sni

ng: The practice of capturing and inspecting data packets across a
...
ack where a malicious actor connects to a

network hub and looks at all tra

c on the network

Patch update: A so

ware and operating system update that addresses security

vulnerabilities within a program or product

Penetration testing: A simulated a

ack that helps identify vulnerabilities in systems,

networks, websites, applications, and processes
Ping of death: A type of DoS a

ack caused when a hacker pings a system by sending

it an oversized ICMP packet that is bigger than 64KB
Port: A so

ware-based location that organizes the sending and receiving of data

between devices on a network
Port
ltering: A

rewall function that blocks or allows certain port numbers to limit

unwanted communication
Proxy server: A server that ful

lls the requests of its clients by forwarding them to
...
ack performed when a malicious actor intercepts a data
...
S
Secure File Transfer Protocol (SFTP): A secure protocol used to transfer
...
oods it with ICMP packets

Speed: The rate at which a device sends and receives data, measured by bits per
second
Stateful: A class of

rewall that keeps track of information passing through it and

proactively

lters out threats
Stateless: A class of

rewall that operates based on prede

ned rules and that does not

keep track of information from data packets
Subne
ing: The subdivision of a network into logical groups called subnets
Switch: A device that makes connections between speci

c devices on a network by
...
ack that simulates a TCP/IP

connection and

oods a server with SYN packets

T
TCP/IP model: A framework used to visualize how data is organized and transmi
ed
...
<img width="773" height="418" alt="image" src="https://github.com/user-attachments/assets/12bf2065-1ed9-4e11-8d23-db4500f307b8" />
...
<img width="784" height="445" alt="image" src="https://github.com/user-attachments/assets/d4b58e92-631c-4613-9496-4d3b1068b1ba" />
...
<img width="784" height="445" alt="image" src="https://github.com/user-attachments/assets/034f3fef-beb8-4f54-b692-ea8e445ac3d4" />
...
<img width="815" height="446" alt="image" src="https://github.com/user-attachments/assets/5c3d029d-c071-4960-ad23-a7c0bc3073fa" />
...
## **Ongoing Monitoring of CI/CD — Automated Threat Detection**

### **Why Monitoring CI/CD Matters**

* **CI/CD pipelines** speed up software delivery but can be a **target for attackers**.
* If compromised, attackers can:

  * Inject malicious code.
  * Steal sensitive data (like secrets).
  * Break or disrupt deployments.
* **Goal:** Detect threats early with automation before serious damage.

---

## **Automation for Finding Threats**

* Goes beyond just collecting logs.
* Uses **tools + machine learning/rules** to detect:

  * Unusual code changes.
  * Strange deployments.
  * Anomalous execution patterns.
  * Attempts to access secrets.
* **Benefit:** Faster detection → quicker response → less damage.
...
1. **Unauthorized Code Changes**
   ...
   * Commits from unexpected users.
   * Code edits at odd times/locations.
   * Suspicious code (big deletions, obfuscated logic).

2. **Suspicious Deployment Patterns**

   * Deployments to unapproved systems.
   * Deployments outside normal schedule.
   * Deployments triggered by unusual accounts.

3. **Compromised Dependencies**

   * Vulnerable packages (CVEs) found.
   * Sudden new dependencies.
   * Downloads from untrusted sources.

4. **Unusual Pipeline Execution**

   * Steps failing unexpectedly.
   * Runs taking much longer.
   * Order of steps changed without approval.

5. **Secrets Exposure Attempts**

   * Logs showing attempts to read secrets.
   * Hardcoded secrets in commits.

---

## **Methods to Monitor & Detect Automatically**

### **1. Comprehensive Logging & Auditing**

* **Pipeline Execution Logs:**
  Build a baseline of “normal” execution times & success rates → detect deviations.
* **Code Commit Logs:**
  Flag suspicious or unauthorized commits.
* **Access Logs:**
  Detect unusual logins, failed logins, and country/location mismatches.
* **Deployment Logs:**
  Monitor frequency, timing, and target environments.

### **2. SIEM Integration**

* **SIEM (Security Information & Event Management)** collects CI/CD logs.
* Detects anomalies automatically.
* Uses **rules + threat intelligence** to alert on:
...
* For:
  ...
  * Highly suspicious code changes.
  * Secrets access attempts.
  * Odd network traffic from CI/CD servers.

### **4. Performance Monitoring**

* Detects **Indicators of Attack (IoAs)** like:

  * Slowdowns.
  * High resource usage.
* These may reveal deeper **IoCs**.

### **5. Continuous Vulnerability Scanning**

* Regular scans for:

  * Outdated plugins/tools.
...
* **Faster Incident Response:** Quick action before attackers succeed.
* **Damage Limitation:** Reduces impact time window.
* **Threat Intelligence Growth:** Learn attacker patterns for future prevention.
...
* **Definition:** Observable evidence suggesting a possible security incident (e.g., suspicious file names, malicious IP addresses).
* **Purpose:** Helps identify **who** attacked and **what** happened **after** the attack.
* **Example:**

  * IoC: The malicious file name found on the system.
  * IoA: The process that downloaded and executed that file.

**⚠ Note:** IoCs don’t always mean an attack — they can result from human mistakes or system errors.

---

### **2. Indicators of Attack (IoAs)**

* **Definition:** Signs that an attack is *in progress* or about to happen, focusing on attacker behavior, methods, and intent.
* **Purpose:** Helps identify **why** and **how** an attack is happening, in real time.
* **Example:**

  * IoA: A process making an unusual network connection to an external server.
  * IoC: The IP address of that server.

---

### **3. Pyramid of Pain**

* **Goal:** Shows the **difficulty** attackers face when you block certain IoCs.
* **Idea:** The higher up the pyramid you block, the more “pain” for the attacker and the harder it is for them to adapt.
* **Levels (bottom = easiest for attacker to bypass, top = hardest):**

  1. **Hash values** – Unique identifiers for files (easy to change).
  2. **IP addresses** – Attackers can quickly switch to new ones.
  3. **Domain names** – Can be replaced, but takes more effort.
  4. **Network artifacts** – Patterns in network traffic; harder to change without altering attack infrastructure.
  5. **Host artifacts** – Things left on a machine (files, registry keys); harder to hide consistently.
  6. **Tools** – Software attackers use; replacing tools can be a big setback.
  7. **Tactics, Techniques, and Procedures (TTPs)** – Attacker’s overall behavior and strategy; hardest to change and adapt.

---

### **4. Key Takeaways**

* IoCs = *evidence after the fact*; IoAs = *evidence during the act*.
* The **Pyramid of Pain** helps prioritize detection methods.
* Blocking high-level IoCs (like TTPs) forces attackers to change their entire approach — a major win for defenders.
